  ...
  def install_plugin name, params={}
    ...
    ruby_block "Install plugin: #{name}" do
      block do
        ...
        command = "#{node['es']['bindir']}/plugin install #{name}#{version}#{url}"
	puts "\nCOMMAND IS: #{command}\n"
        ...
        raise "[!] Failed to set permission" unless system "chown -R #{node['es']['user']}:#{node['es']['user']} #{node['es']['path.home']}/plugins/"
      end

#      notifies :restart, 'service[elasticsearch]' unless node.elasticsearch[:skip_restart]

      not_if do
        Dir.entries("#{node['es']['path.home']}/plugins/").any? do |plugin|
          next if plugin =~ /^\./
          name.include? plugin
        end rescue false
      end
